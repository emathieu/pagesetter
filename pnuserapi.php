...
function pagesetter_userapi_getPubFormatted($args)
{
  ...
  if (isset($args['expose_template'])) $smarty->expose_template = $args['expose_template'];
...
function escapePublicationForHTML(&$pubInfo, &$pub, $useTransformHooks, $uniqueId)
{
  foreach ($pub as $key => $value)
  {
    if (is_string($value))
    {
// EM Commented because of some issues with PN 0.8 and fields containing <a href="...">...</a>
...
//EM  list($pub['core']['title']) = pnModCallHooks('item', 'transform', $uniqueId, array($pub['core']['title']));
}
