...
class pubListHandler extends MenuHandler
{
  ...
  function action($event)
  {
    ...
    else if ($event['action']['action'] == 'insert') // Insert link to publication (when used as "find publication" browser)
    {
      ...
      $title = htmlspecialchars($title, ENT_QUOTES); //EM
...
//EM      $this->commander->raw("<script>pagesetterPasteLink('absolute', 'a', '$title', '$url', '" 
//EM
//EM                            . $event['extra']['targetID'] . "', 'htmlArea30');</script>\n");
//EM begin: for Publication field type
      $this->commander->raw("<script>pagesetterPasteLink('"
                            . $event['extra']['URLMode']    . "', 'a', '$title', '$url', '"
                            . $event['extra']['targetID']   . "', '"
                            . $event['extra']['targetMode'] . "');</script>\n");
//EM end
