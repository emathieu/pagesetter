...
function pagesetter_erictag_view($args) {
	return pagesetter_eric_main($args);
}

function pagesetter_erictag_main($args) {
	...
	foreach ($pubList['publications'] as $pub) {
		...
		$pubdata = pnModAPIFunc('pagesetter', 'user', 'getPub', array (
			...
			// Enfants (3)
			// Caroline (4)
			if ($pubdata['ma_categorie'] == 4){
echo $pub['pid']."<br />";
				pnModAPIFunc('pagesetter', 'relations', 'setRelations', array (
					'tid' => $tid,
					'pid' => $pub['pid'],
					'targetTid' => 9,
					'targetPids' => array(1 => 2)
				));

			}
			// Natasha (8)
			// Benjamin (9)
			if ($pubdata['ma_categorie'] == 9){
			}
			// Sorties (5)
			// Cinéma (10)
			// Concert (11)
