...
// Patch Eric Mathieu to manage relation within the same PID
//BEGIN
    if ($this->tid == $this->targetTid) {
  	...
  	$join .= "\nLEFT JOIN $relationsTable AS $alias ON " .
                                "($relationsColumn[targetTid] = '" . pnVarPrepForStore($this->tid) . "' " .
                             "AND $relationsColumn[targetField] = '" . pnVarPrepForStore ($this->ftid) . "' " .
                             "AND $relationsColumn[targetPid] = $tableName.$tableColumns[pid] " .
  			                 "AND $relationsColumn[sourceTid] = '" . pnVarPrepForStore($this->targetTid) . "'";
  			                 
    if ($this->targetField != '')
      $join .=  " AND $relationsColumn[sourceField] = '" . pnVarPrepForStore($this->targetField) . "'";
    ...
    $join .= ")";
     ...
     switch ($operator) {
    	  case 'eq':
    	  case 'rel': 
	     $sql .= " OR $relationsColumn[sourcePid] = '" . pnVarPrepForStore($value) . "'";
    	    break;
    	  case 'ne':
    	  case 'nrel':
    	    $sql .= " AND $relationsColumn[sourcePid] != '" . pnVarPrepForStore($value) . "'";
    	    break;
          case 'null':
            $sql .= " OR $relationsColumn[sourcePid] IS NULL";
            break;
          case 'notnull':
            $sql .= "AND $relationsColumn[sourcePid] IS NOT NULL";            
            break; 
...
//END
  ...
  function OnPublicationCreated($pubData)
  {
  	$this->parseTypeData();
  ...
  function OnPublicationUpdated($pubData)
  {
 	return $this->OnPublicationCreated($pubData);
  }
  
  function OnPublicationDeleted($pubData)
  {
