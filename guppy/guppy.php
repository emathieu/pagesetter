<?php
// $Id: guppy.php,v 1.134 2007/02/08 21:30:42 jornlind Exp $
// =======================================================================
...
function guppy_invokeEvent($eventName, &$args)
{
  ...
  guppy_executeEvent($windowData['spec'], $eventName, $args, $windowData['data']);
}


function guppy_executeEvent(&$fullSpec, $eventName, &$args, &$data)
{
  ...
  foreach ($spec as $componentName => $componentSpec)
  {
    if ($componentSpec['kind'] == 'card')
    {
      guppy_executeComponentEvent($componentName, $componentSpec, 0, $eventName, $args, $data); //FIXME
    }
    else
    if ($componentSpec['kind'] == 'table')
    {
      ...
      for ($rowIndex=0; $rowIndex<$rowCount; ++$rowIndex)
      {
        guppy_executeComponentEvent($componentName, $componentSpec, $rowIndex, $eventName, $args, $data); // FIXME
      }
    }
    else
    if ($componentSpec['kind'] == 'tree')
...
function guppy_executeComponentEvent(&$componentName, &$componentSpec, $rowIndex, $eventName, &$args, &$data)
{
	// Patch EM to enable acces to publication data	
	$ldata  = $data['pubedit']['rows'][0];
	$largs = $args;
	$largs['data']  = &$ldata;
	...
	foreach ($componentSpec['fields'] as $field)
	{
		if (isset($field['inUse'])  &&  $field['inUse'])
		{
			...
			else // plugin
			{
				...
				{
					// Patch EM to enable acces to publication data	
					call_user_func(array(&$plugin, $eventName), $largs);
					// End of patched section
				}
			}
		}
...
// end scribite!      
  ...
  else // Plugin
  {
    ...
    echo $plugin->render(new GuppyPluginHelper, $fieldSpec, $element, isset($componentSpec['actions']['submit']));
  }
