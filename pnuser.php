...
function pagesetter_user_main($args)
{
  ...
  if (pnModLoad('mediashare'))
  {
    // Use PostNuke variable for insertion of special HTML header tags.
    // - in this case the JavaScript used for Photoshare popup images.
    global $additional_header;
    $additional_header[] = "<script type=\"text/javascript\" src=\"modules/mediashare/pnjavascript/view.js\"></script>";
  }
...
function pagesetter_user_publist()
{
  ...
  if (!guppy_decode($handler))
  {
    ...
    guppy_open( array( 'specFile'       => 'modules/pagesetter/forms/pubListSpec.xml',
                       ...
                       'options'        => $options,
                       'data'           => $data,
                       'extra'          => $extra, //EM: for Publication field type
...
function pagesetter_user_pubfind($args)
{
  ...
  if (!guppy_decode($handler))
  {
    ...
    $URLMode = pnVarCleanFromInput('url');       //EM: for Publication field type
    $targetMode = pnVarCleanFromInput('target'); //EM: for Publication field type
    if (!isset($targetID))
    ...
    $extra = array( 'targetID'   => $targetID,
                    'URLMode'    => $URLMode,      //EM: for Publication field type
                    'targetMode' => $targetMode ); //EM: for Publication field type
